...
func TestDeployCounter(t *testing.T) {
	...
	out, err := cli.Run("deploy", "-name", "CounterExample", "-code", "./_counter/contract.go", "-env", getGammaEnvironment())
	t.Log(out)
	require.NoError(t, err, "deploy should succeed")
	require.True(t, strings.Contains(out, `"ExecutionResult": "SUCCESS"`))

	out, err = cli.Run("read", "-i", "counter-get.json", "-env", getGammaEnvironment())
	t.Log(out)
	require.NoError(t, err, "get should succeed")
	require.True(t, strings.Contains(out, `"ExecutionResult": "SUCCESS"`))
	require.True(t, strings.Contains(out, `"Value": "0"`))

	out, err = cli.Run("send-tx", "-i", "counter-add.json", "-env", getGammaEnvironment())
	t.Log(out)
