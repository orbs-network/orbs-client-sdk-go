...
func TestDeployCounter(t *testing.T) {
	...
	out, err := cli.Run("deploy", "-env", "local", "-config", "./orbs-gamma-config.json", "-name", "CounterExample", "-code", "./counter/contract.go")
	t.Log(out)
	require.NoError(t, err, "deploy should succeed")
	require.True(t, strings.Contains(out, `"ExecutionResult": "SUCCESS"`))

	out, err = cli.Run("read", "-env", "local", "-config", "./orbs-gamma-config.json", "-i", "counter-get.json")
	t.Log(out)
	require.NoError(t, err, "get should succeed")
	require.True(t, strings.Contains(out, `"ExecutionResult": "SUCCESS"`))
	require.True(t, strings.Contains(out, `"Value": "0"`))

	out, err = cli.Run("send-tx", "-env", "local", "-config", "./orbs-gamma-config.json", "-i", "counter-add.json")
	t.Log(out)
